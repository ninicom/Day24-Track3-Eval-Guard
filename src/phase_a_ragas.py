...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    """
    # Preload heavy native libs first (torch before others) to avoid a Windows
    # init-order segfault during RAGAS. Done here, not at module top, so the pure
    # Phase A functions stay importable under pytest without loading torch.
